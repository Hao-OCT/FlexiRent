...
public class CompleteController implements Initializable {
	private ObservableList<String> completeList;
	@FXML
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		for (int i = 0; i < MainController.propertyList.size(); i++) {
			if (MainController.propertyList.get(i).getPropertyStatus().compareTo("Maintenance") == 0) {
				completeList.add(MainController.propertyList.get(i).getPropertyID());
			}
		}
		// initialize combo box values
		cbPID.setValue(null);
		cbPID.setItems(completeList);
	...
	public void goNext(ActionEvent event) {
		...
		try {
			...
			date = comDate.getValue().format(DateTimeFormatter.ofPattern("dd-MM-yyyy"));
			...
			if (index != -1) {
				...
				output.setText("Complete Maintenance Successfully");
			} }
			else {
				throw new CompleteException("You have not selected property ID, if you have nothing to choose it means no property can be completed maintenance right now");
			}

		} catch (CompleteException pe) {
